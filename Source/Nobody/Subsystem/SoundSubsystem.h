...
UCLASS()
class NOBODY_API USoundSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
public:
	...
	void PlaySFXInLocation(USoundCue* InSound, const FVector& Location);

	/** 위치와 관계없이 제자리에서 SFX를 재생하는 함수입니다. */
	UFUNCTION(BlueprintCallable)
	void PlaySFX2D(ESFX SFXType);
	void PlaySFX2D(USoundCue* InSound);
...
private:
	...
	/** GC 방지용 오디오 컴포넌트 배열입니다. */
	UPROPERTY()
	TArray<UAudioComponent*> GCPreventionArray;
